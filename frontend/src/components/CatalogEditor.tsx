...
import {
  ...
  importCatalogFromMailbox,
  exportCatalogToMailbox,
...
export default function CatalogEditor({ embedded = false, showDevtools = !embedded }: CatalogEditorProps): JSX.Element {
  ...
  const handleImportMailbox = useCallback(async () => {
    ...
    try {
      ...
      setStatus({ kind: 'error', message: `Import fehlgeschlagen: ${toMessage(err)}` })
    } finally {
      setSyncBusy(null)
    }
  }, [])
  ...
  const handleExportMailbox = useCallback(async () => {
    ...
    try {
      ...
      const createdCount = response.created_folders.length
      const message = createdCount
        ? `${createdCount} Ordnerpfade geprüft oder angelegt.`
        : 'Katalogordner wurden geprüft.'
      setStatus({ kind: 'success', message })
    } catch (err) {
      setStatus({ kind: 'error', message: `Export fehlgeschlagen: ${toMessage(err)}` })
    } finally {
      setSyncBusy(null)
    }
  }, [])
